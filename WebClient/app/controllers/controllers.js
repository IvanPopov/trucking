...
//It's used to track the orderby parameter and ordersTotal for a customer. Put it here rather than duplicating 
//setOrder and orderby across multiple controllers.
app.controller('CreateOrder', function ($scope, usersService, clientsService) {
    ...
    function init() {
        ...
        $scope.CurrentUserId = usersService.getCurrentUserId();
        $scope.Author = usersService.getUser($scope.CurrentUserId);

        $scope.Clients = clientsService.getClients();
        $scope.Client = clientsService.getDefaultClient();

        $scope.Holdings = clientsService.getHoldings();
