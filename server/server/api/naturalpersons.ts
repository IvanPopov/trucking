...
function init(app: express.Express, log: winston.Logger) {
	 ...
	 *     }
	 */
	app.get("/api/naturalpersons/:id/status",
		passport.authenticate("bearer", { session: false }),
		(req, res, done) => {
			db.naturalpersons.findRow({ id_naturalperson: parseInt(req.params.id) || 0 }, (e: Error, person: trucking.db.INaturalPerson) => {
				if (e || !person) {
					return res.json(404, { error: "Person not found." });
				}

				if (person.fired) {
					res.json({ status: trucking.db.ENaturalPersonStatuses.NOT_WORKING });
				}
				else {
					res.json({ status: trucking.db.ENaturalPersonStatuses.UNKNOWN });
				}
			});
		});
