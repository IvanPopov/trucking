...
function init(app: express.Express, log: winston.Logger) {
	...
	app.post("/api/catalogs/tools/groups",
		passport.authenticate("bearer", { session: false }),
		(req, res, done) => {
			var check = revalidator.validate(req.body, {
				properties: {
			...
			db.catalogs.toolgroups.create(req.body, (err, result) => {
				if (err) return done(err);
				res.json(201, result);
			});
		});
	 ...
	 * @apiSuccessStructure Deleted
	 */
	app.del("/api/catalogs/tools/groups/:group",
		passport.authenticate("bearer", { session: false }),
		(req, res, done) => {
			var cond = {};
			...
			db.catalogs.toolgroups.del(cond, (err) => {
				if (err) return done(err);
				res.json(204, null);
			});
		});
	...
	app.get("/api/catalogs/tools",
		passport.authenticate("bearer", { session: false }),
		(req, res, done) => {
			if (type.isString(req.query.group)) {
				var cond = { id_toolgroup: parseInt(req.query.group) || 0 };
				db.catalogs.tools.find(cond, (err: Error, tools: trucking.db.ITool[]) => {
					if (err) return done(err);
					res.json(tools);
				}, req.query);
			}
			else {
				db.catalogs.tools.get(
					(err: Error, tools: trucking.db.ITool[]): void => {
						if (err) return done(err);
						res.json(tools);
					}, req.query);
			}
